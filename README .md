...
![NeoClaw Logo](docs/neo_logo.png)

# NeoClaw

###  **当 AI 开始"黑箱操作"，你需要一双透视眼**

[![NeoClaw](https://img.shields.io/badge/NeoClaw-1.0.0-purple.svg?logo=cyberpunk)](https://github.com/ttguy0707/NeoClaw)
...
> 🤖 **你的 AI 在背着你做什么？NeoClaw 让所有行为无所遁形**
> 
> 💡 **灵感来源**：受 [OpenClaw](https://github.com/openclaw/openclaw) 的启发，NeoClaw 专注于解决 AI 智能体的透明度和可控性问题。
...
NeoClaw 是一个**企业级透明可控智能体**，重新定义 AI 系统的可信边界：
...
NeoClaw 支持**OpenClaw 技能**和**Claude Code 技能**，可直接使用两个生态系统的丰富技能资源，无需重新开发。
...
- **推荐技能**：
  - `skill-creator`：用自然语言让 NeoClaw 自己创建技能
...
```bash
# 克隆项目
git clone https://github.com/ttguy0707/NeoClaw.git
cd NeoClaw
...
> 💡 **工作区配置**：工作区路径已在代码中初始化，默认为项目根目录的 `workspace` 文件夹，无需在 `.env` 中配置。仅当需要自定义工作区位置时，才设置 `NEOCLAW_WORKSPACE` 环境变量。

> 💡 提示：配置完成后，可运行 `neoclaw run` 聊天测试连接是否正常。
...
| 模块 | 文件 | 功能 |
|------|------|------|
| **Agent 循环** | `neoclaw/core/agent.py` | LangGraph StateGraph，决策大脑 |
| **技能加载** | `neoclaw/core/skill_loader.py` | 动态加载 SKILL.md，两段式调用 |
| **上下文管理** | `neoclaw/core/context.py` | 消息修剪，双水位记忆 |
| **内置工具** | `neoclaw/core/tools/builtins.py` | 时间/计算/任务调度等 |
| **沙盒工具** | `neoclaw/core/tools/sandbox_tools.py` | 文件操作 + Shell 执行 |
| **审计日志** | `neoclaw/core/logger.py` | JSONL 格式事件记录 |
| **心跳任务** | `neoclaw/core/heartbeat.py` | 定时任务检查与触发 |
...
[![Star History Chart](https://api.star-history.com/svg?repos=ttguy0707/NeoClaw&type=Date)](https://star-history.com/#ttguy0707/NeoClaw&Date)
...
> 🤖 **What is your AI doing behind the scenes? NeoClaw makes every action visible.**
>
> 💡 **Inspired by** [OpenClaw](https://github.com/openclaw/openclaw), NeoClaw focuses on transparency and controllability for AI agents.

---

## 📖 Introduction

NeoClaw is an **enterprise-grade transparent and controllable agent** that redefines the trust boundary of AI systems:
...
NeoClaw supports both **OpenClaw skills** and **Claude Code skills**, so you can reuse rich skill resources from both ecosystems without rebuilding them.
...
- **Recommended skills**:
  - `skill-creator`: create skills with natural language through NeoClaw
...
> ```
>
> After installation, the `neoclaw` command is available from any directory.
...
> 💡 **Workspace configuration**: the workspace path is initialized in code and defaults to the `workspace` folder in the project root. You do not need to configure it in `.env`. Set the `NEOCLAW_WORKSPACE` environment variable only when you need a custom workspace path.

> 💡 Tip: after configuration, run `neoclaw run` to test whether chat connectivity works.
...
After startup, NeoClaw enters the interactive chat interface:
...
NeoClaw includes a heartbeat task system that automatically executes scheduled tasks in the background:
...
- **Real-time monitoring**: run `neoclaw monitor` to view task execution logs
...
|------|------|------|
| **Agent loop** | `neoclaw/core/agent.py` | LangGraph StateGraph and decision engine |
| **Skill loading** | `neoclaw/core/skill_loader.py` | Dynamically loads SKILL.md with two-phase invocation |
| **Context management** | `neoclaw/core/context.py` | Message trimming and dual-watermark memory |
| **Built-in tools** | `neoclaw/core/tools/builtins.py` | Time, calculation, task scheduling, and more |
| **Sandbox tools** | `neoclaw/core/tools/sandbox_tools.py` | File operations and shell execution |
| **Audit logging** | `neoclaw/core/logger.py` | JSONL event logging |
| **Heartbeat tasks** | `neoclaw/core/heartbeat.py` | Scheduled task checking and triggering |

### Project Structure

```
NeoClaw/
├── neoclaw/                    # Core package
│   ├── core/
│   │   ├── agent.py              # Agent loop
...
# Then ask NeoClaw to create a new skill with natural language
...
# Ask NeoClaw to check skill safety
...
**👾 NeoClaw · Next-Gen Transparent Agent Architecture**
