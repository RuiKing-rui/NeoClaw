...
from neoclaw.core.agent import create_agent_app
from neoclaw.core.config import DB_PATH
from neoclaw.core.bus import task_queue
from neoclaw.core.heartbeat import pacemaker_loop
...
{RESET}"""
    ...
    sub_title = f"{WHITE}{BOLD} 👾 Welcome to the {PURPLE}{BOLD}NeoClaw{RESET}{WHITE}{BOLD} !  {RESET}"
    ...
    tip = (
        f"{PURPLE} ✦ {RESET}"
        f"{SILVER}{PURPLE}{BOLD}NeoClaw{RESET} 已完成启动。输入命令开始，输入 {PURPLE}/exit{RESET}{SILVER} 退出。{RESET}\n"
    )
...
async def async_main():
    ...
    # 创建agent，LangGraph Checkpointer负责把 Agent State 保存进SQLite，下一轮对话还能恢复
    # checkpoint里面存放的thread_id用来区分不同用户或不同会话
        ...
        async def user_input_loop():
            ...
            while True:
                try:
                    ...
                    if user_input.lower() in ["/exit", "/quit"]:
                        cprint("  \033[38;5;141m✦ 记忆已固化，NeoClaw 进入休眠。\033[0m")
                        break
                        
                except (KeyboardInterrupt, EOFError):
                    cprint("\n  \033[38;5;141m✦ 强制中断，NeoClaw 进入休眠。\033[0m")
