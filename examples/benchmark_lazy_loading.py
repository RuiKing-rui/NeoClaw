...
from neoclaw.core.skill_loader import load_dynamic_skills, get_skill_count, clear_skill_cache
...
def benchmark():
    """性能基准测试"""
    print("=" * 70)
    print("NeoClaw 懒加载性能基准测试")
    print("=" * 70)
    
    # 创建临时测试目录
    temp_dir = tempfile.mkdtemp(prefix="neoclaw_bench_")
    
    # 修改环境变量
    original_env = os.environ.get("NEOCLAW_WORKSPACE")
    os.environ["NEOCLAW_WORKSPACE"] = temp_dir
    ...
    try:
        ...
        for num_skills in test_sizes:
            ...
            # 重新加载配置
            import importlib
            import neoclaw.core.config as config_module
            import neoclaw.core.skill_loader as skill_loader_module
        ...
        if original_env is not None:
            os.environ["NEOCLAW_WORKSPACE"] = original_env
        else:
            os.environ.pop("NEOCLAW_WORKSPACE", None)
