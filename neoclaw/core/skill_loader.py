#负责把每个 Skill 目录包装成一个 StructuredTool
import os
import re
import time
...
#这是 Pydantic 参数模型，也就是所有动态 Skill 共用的工具 Schema
#所有动态 Skill 的统一调用协议，LLM 调用时统一使用DynamicSkillInput对象
...
# 启动时只扫描 name / description，使用时再加载完整内容
class LazySkillLoader:
    ...
    def __init__(self, cache_size: int = 50):
        self._skill_registry: Optional[List[Dict[str, Any]]] = None     #扫描出来的技能元数据
        self._cache_size = cache_size                                   #声明的内容缓存大小
        self._last_scan_time = 0                                        #上次扫描技能目录的时间
        self._scan_interval = 60                                        # 缓存元数据扫描结果60秒

    #完整说明书缓存
    #第一次调用：读取磁盘。使用相同参数再次调用：直接返回缓存，最多缓存 50 份说明书
    #相同路径，修改时间相同，使用旧缓存
    ...
    def _scan_skills(self, force_rescan: bool = False) -> List[Dict[str, Any]]:
        ...
        #扫描：workspace/office/skills/里面所有文件夹
        for item in sorted(os.listdir(SKILLS_DIR)):
            ...
            try:
                # 只读取前几行（name, description），Python正则化只提取name和description，返回对应字典
                ...
                if metadata:
                    skills.append({
                        "folder": item,
                        "md_path": md_path,
                        "mtime": os.path.getmtime(md_path),     #记录文件修改的时间
                        **metadata
                    })
